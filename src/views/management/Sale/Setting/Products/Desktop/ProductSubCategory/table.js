...
const ProductSubCategoryTable = props => {
  ...
  useEffect(() => {
    setColumns([
      ...
      {
        name: 'category_detail.translate_detail.item_fa',
        label: 'دسته',
        options: {
          filter: true,
          filterType: 'custom',
  ...
  function getData(page, rowsPerPage, search) {
    httpService
      .post(
        `${API_BASE_URL}/api/management/product/subcategory/subcategory_list/?limit=${rowsPerPage}&offset=${page *
  ...
  function onFilterChange(column, filterList, type) {
    ...
    switch (column) {
      case 'name':
        ...
        break;
      case 'category_detail.translate_detail.item_fa':
        if (filterList[2][0]) {
          item['category__name'] = filterList[2][0];
          filterType = '__icontains';
        } else {
          delete item['category__name'];
        }
        break;
  ...
  function onColumnSortChange(changedColumn, direction) {
    ...
    switch (changedColumn) {
      case 'name':
        ...
        break;
      case 'category_detail.translate_detail.item_fa':
        itemSort['category__name'] = direction;
        break;
      case 'translate_detail.item_en':
  ...
  function onRowClick(rowData, rowState) {
    history.push({
      pathname: '/management/sale/setting/products/subCategory',
      state: {
  ...
  function onRowsDelete(rowsDeleted, newData) {
    ...
    rowsDeleted.data.map((item, index) => {
      subCatIds.push(data[item.index].id);
    });

    httpService
      .post(
        `${API_BASE_URL}/api/management/product/subcategory/subcategory_delete/`,
        {
