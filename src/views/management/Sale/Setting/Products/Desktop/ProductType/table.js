...
let item = {};
// let itemSort = {};
const ProductSubCategoryTable = props => {
  ...
  useEffect(() => {
    setColumns([
      ...
      {
        name: 'subcategory_detail.translate_detail.item_fa',
        label: 'زیر دسته',
        options: {
          filter: true,
          filterType: 'custom',
  ...
  function getData(page, rowsPerPage, search) {
    httpService
      .post(
        `${API_BASE_URL}/api/management/product/type/type_list/?limit=${rowsPerPage}&offset=${page *
  ...
  function onFilterChange(column, filterList, type) {
    ...
    switch (column) {
      case 'name':
        ...
        break;
      case 'subcategory_detail.translate_detail.item_fa':
        if (filterList[2][0]) {
          item['subcategory__translate__item_fa'] = filterList[2][0];
          filterType = '__icontains';
        } else {
          delete item['subcategory__translate__item_fa'];
        }
        break;
      case 'name_translate.item_en':
        if (filterList[3][0]) {
      ...
      case 'name_translate.item_fa':
        ...
        break;
      case 'name_translate.item_ar':
        if (filterList[5][0]) {
          item['translate__item_ar'] = filterList[5][0];
          filterType = '';
        } else {
          delete item['translate__item_ar'];
        }
        break;
      case 'name_translate.item_ru':
        if (filterList[6][0]) {
          item['translate__item_ru'] = filterList[6][0];
          filterType = '';
        } else {
      ...
      case 'active_in_shop':
        if (filterList[8][0]) {
          item['active_in_shop'] =
            filterList[8][0] == 'فعال' ? 'True' : 'False';
          filterType = '';
        } else {
  ...
  function onColumnSortChange(changedColumn, direction) {
    ...
    switch (changedColumn) {
      case 'name':
        ...
        break;
      case 'subcategory_detail.translate_detail.item_fa':
        itemSort['subcategory__translate__item_fa'] = direction;
        break;
      case 'name_translate.item_en':
  ...
  function onRowClick(rowData, rowState) {
    history.push({
      pathname: '/management/sale/setting/products/type',
      state: {
  ...
  function onRowsDelete(rowsDeleted, newData) {
    ...
    httpService
      .post(`${API_BASE_URL}/api/management/product/type/type_delete/`, {
        type_ids: typeIds
      })
      .then(res => {
        if (res.status === 200) {
