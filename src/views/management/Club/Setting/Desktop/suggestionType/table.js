...
const SuggestionTypeTable = props => {
  ...
  function getData(page, rowsPerPage, search) {
    httpService
      .post(
        `${API_BASE_URL}/api/management/club/suggestion_type/suggestion_type_list/?limit=${rowsPerPage}&offset=${page}${
          filter !== '' ? `&${filter}` : ''
        }`,
        {
          order: sort,
  ...
  function onRowsDelete(rowsDeleted, newData) {
    ...
    httpService
      .post(`${API_BASE_URL}/api/management/club/suggestion_type/delete/`, {
        type_ids: typeIds
      })
      .then(res => {
        if (res.status === 200) {
            ...
            <Box>
              <Formik
                ...
                onSubmit={(values, { setErrors, setSubmitting }) => {
                  ...
                  httpService
                    .post(
                      `${API_BASE_URL}/api/management/club/suggestion_type/`,
                      values
                    )
                    .then(res => {
                      if (res.status === 201) {
...
export default SuggestionTypeTable;
